...
while running:
    ...
    for k in range(3):
            ...
            #Solucuion de ecuacion diferencial ordinaria
    ...
    K13 = ode1(x3,u3,m3)
    K23 = ode1(x3+0.5*hsim*K13,u3,m3)
    K33 = ode1(x3+0.5*hsim*K23,u3,m3)
    K43 = ode1(x3+1.0*hsim*K33,u3,m3)
    x3  = x3 + (1/6)*hsim*(K13 + 2*K23 + 2*K33 + K43)
    ...
    #MOSTRAR POSICION DE MASA m3
    ppx = x3[0,0] * mlt
    ppy = x3[2,0] * mlt
    ppz = x3[4,0] * mlt
    prx = ppx*R[0,0] + ppy*R[0,1] + ppz*R[0,2] + cn
    pry = ppx*R[1,0] + ppy*R[1,1] + ppz*R[1,2] + cn
    scralp6 = pygame.Surface((500,500), pygame.SRCALPHA)
    pygame.draw.circle(scralp6,(10,10,255,130),
                       (prx,pry),3)
    screen.blit(scralp6,(0,0))
    ...
    str(u3[0]) + str(u3[1]) + str(u3[2]), False, (0,0,0))
